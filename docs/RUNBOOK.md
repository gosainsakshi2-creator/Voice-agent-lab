...
| Call watchdogs | `CAMPAIGN_RING_TIMEOUT_SECONDS`, `CAMPAIGN_MAX_CALL_SECONDS`, `CAMPAIGN_MAX_SILENCE_SECONDS`, `CAMPAIGN_CLOSING_WAIT_SECONDS` (default 8; how long a confirmed registration waits for the person's closing word before hanging up anyway — keep it below the silence window) |
