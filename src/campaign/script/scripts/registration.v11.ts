 ...
 * IN 10 MINUTES", 22 September 2026, 7:30 PM IST. Revision 11.
 *
 * A NEW immutable version. v1-v10 stay byte-identical, so every campaign
 ...
 *
 * ── WHAT CHANGED FROM v10, AND WHY IT IS A VERSION AND NOT AN EDIT ─
 *
 * TWO changes, both in how the SAME conversation is framed. Every fact,
 * discovery question, FAQ answer (including the 90-minute duration and
 * the WhatsApp wording), the gate line, the confirmation and the
 * closings are carried over from v10 word for word;
 * `registration-v11-tests` section H asserts that the only lines that
 * differ are the ones named here.
 *
 *   1. FIRST NAME AFTER THE OPENING. `{{customer_name}}` is the imported
 *      name column, which on real lists is often a FULL name ("Rahul
 *      Sharma"), and v10 interpolated it into every spoken line, so the
 *      agent said "Perfect, Rahul Sharma" and "Thanks for your time,
 *      Rahul Sharma". v11 keeps the full name where it does the work —
 *      the opening's identity question — and everywhere after that the
 *      lines carry the marker "[first name]", which the appendix defines
 *      ONCE as the first word of {{customer_name}}. No variable, parser
 *      or data transformation was added: the campaign layer still
 *      supplies exactly `customer_name` and `agent_name`, and the model
 *      does the one-word derivation the way a person would.
 *
 *   2. AN INVITATION, NOT A REMINDER. v10's first reply said "you'd
 *      shown interest… are you still planning to join?", which frames
 *      the call as a follow-up on a decision the person already made.
 *      This campaign calls leads to GET THEM REGISTERED, so v11's first
 *      reply invites: "we're inviting people to our upcoming webinar…
 *      Would you like to join?" / "humara ek upcoming webinar hai… Kya
 *      aap isme join karna chahenge?". The same classifier reasoning
 *      that protected v9/v10's interest check applies: "would you like
 *      to join" carries a gate OFFER but no gate ACTION and matches no
 *      literal anchor, so a "haan" to it is `affirmative_not_at_gate`,
 *      never a registration; "would you like to ATTEND" and anything
 *      about registering ARE anchors and stay forbidden in the text.
 *      The seat question, its wording and its place are untouched.
 *
 * It is a new version rather than an edit of v10 because a campaign
 * record is already pinned to v10's hash (created 2026-09-21); editing
 * v10 would leave that record pointing at words that no longer exist.
 *
 ...
 * to any of them declines nobody. In particular the invitation is NOT
 * worded "would you like to attend?" in English: "interested in
 * attending", "interested to attend", "like to attend" and "want to
 * attend" ARE anchors, and a yes to any of them one exchange into the
 * call would be recorded as a registration. The English invitation is
 * "Would you like to join?", which matches nothing; the Hinglish "Kya
 * aap isme join karna chahenge?" matches nothing either, and the text
 * below tells the model in as many words which phrasings to avoid.
 *
...
Hi [first name], I'm {{agent_name}}, calling from FlexiFunnels. Actually, I am calling to invite you to our upcoming webinar, "Launch Your Business Online in 10 Minutes". It's on 22nd September at 7:30 PM. Would you like to join?

    In Hinglish: Hi [first name], I'm {{agent_name}}, calling from FlexiFunnels. Actually, humara ek upcoming webinar hai — "Launch Your Business Online in 10 Minutes". Ye 22nd September ko 7:30 PM par hai. Kya aap isme join karna chahenge?
...
[no website] That's exactly what this webinar is for — taking your business online, setting up the website and taking payments, all shown live, step by step.
    ...
    In Hinglish: Toh yeh webinar exactly usi ke liye hai — business ko online setup karna , website set up karna aur payments lena, sab live, step by step dikhaya jaayega.
...
export const REGISTRATION_V11: CampaignScript = {
  id: "registration",
  version: "v11",
  campaignType: "registration",
  label: "Registration v11 (Launch Your Business Online in 10 Minutes webinar, 22 September 7:30 PM — invitation framing, first name after the opening)",
  ...
  systemPromptAppendix: [
    ...
    "September at 7:30 PM, and, if they would like to come, to reserve their free seat. This is",
    "an invitation to register, not a follow-up on anything they decided before. You are not",
    "selling anything, there is nothing to pay for, and there is nothing to push.",
    "",
    ...
    "spoken. Use it where the script puts it and nowhere else: a name in every sentence is",
    ...
    "",
    "    \"Hi [first name], I'm {{agent_name}}, calling from FlexiFunnels. Actually, I am calling to invite you ",
    "     to our upcoming webinar, 'Launch Your Business Online in 10 Minutes'. It's on 22nd",
    "    September at 7:30 PM. Would you like to join?\"",
    "",
    "In Hinglish:",
    "",
    "    \"Hi [first name], I'm {{agent_name}}, calling from FlexiFunnels. Actually, humara ek",
    "    upcoming webinar hai — 'Launch Your Business Online in 10 Minutes'. Ye 22nd September ko",
    "    7:30 PM par hai. Kya aap isme join karna chahenge?\"",
    "",
    "That is ONE reply. Introduce yourself once, there, and never again. You are inviting them",
    "to something new — never say or imply that they signed up, showed interest or decided",
    "anything before this call.",
    "",
    "The question at the end is a real question and their answer decides what comes next. A yes",
    "to it means they want to hear more — it is not the seat question, and it registers nobody —",
    "so go straight on to the question about them. Ask it in those words. Do not turn it into",
    "\"would you like to attend\", \"do you want to attend\", \"are you interested in attending\"",
    "or anything about registering or reserving — those are the words this call uses for the",
    "seat itself, and a \"haan\" to one of them is recorded as a registration before the person",
    "has been told anything. \"Would you like to join?\" and \"Kya aap isme join karna chahenge?\"",
    "are the safe forms, and they are the only ones you use here.",
    "",
    "# THEN, ABOUT THEM",
    "",
    ...
    "in one short line — \"Thanks for your time, [first name]. Have a great day!\" The",
    ...
    "- \"I don't remember this.\" / \"Kaunsa webinar?\" — no problem at all. It is",
